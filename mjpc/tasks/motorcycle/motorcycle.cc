...
#include "mjpc/tasks/motorcycle/motorcycle.h"
...
namespace mjpc {
std::string Motorcycle::XmlPath() const {
  return GetModelPath("motorcycle/task.xml");
}
std::string Motorcycle::Name() const { return "Motorcycle"; }
...
// -------------------------------------------------------------
void Motorcycle::ResidualFn::Residual(const mjModel* model, const mjData* data,
...
//   move goal randomly.
// ---------------------------------------------
void Motorcycle::TransitionLocked(mjModel* model, mjData* data) {
  double* target = SensorByName(model, data, "target");
  double* nose = SensorByName(model, data, "nose");
  ...
  if (mju_norm(nose_to_target, 2) < 0.08) {
    absl::BitGen gen_;
    data->mocap_pos[0] = absl::Uniform<double>(gen_, -10, 10);
    data->mocap_pos[1] = absl::Uniform<double>(gen_, -10, 10);
  }
}
